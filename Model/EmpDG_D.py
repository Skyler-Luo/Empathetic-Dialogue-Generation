...
def extract(g):
    """保存梯度供 WGAN-GP 梯度惩罚计算使用。

    - 参数: 
      - g: 由 autograd 回传的梯度张量
    - 作用: 将 `g` 赋值到全局变量 `xg`，供后续范数计算
    """
    global xg
    xg = g
...
class Semantic_Discriminator(nn.Module):
    ...
    通过共享词嵌入 + 单向 LSTM 提取序列表示，再使用 2-gram 卷积 + MaxPool 提取响应的局部特征，
    融合对话上下文与下一轮反馈隐状态，输出对当前响应是否更“语义合理”的打分（WGAN 判别值，不经 Sigmoid）。

    - vocab: 词表对象，提供 `n_words` 与共享嵌入
    - embedding_size: 词向量维度
    - hidden_size: LSTM 隐层维度（亦作为卷积输出通道数）
    - num_layers: LSTM 层数
    - layer_dropout: LSTM 层间丢弃率
    """
    ...
    def conv2d(self, input, output_dim, k_h, k_w):
        """为时间维做一维卷积（实现为二维卷积），提取 n-gram 局部特征。

        - 输入形状: (batch, 1, seq_len, hidden_size) 经过预处理后为 (batch, hidden_size, 1, seq_len)
        - 参数:
          - output_dim: 输出通道数（通常等于 hidden_size）
          - k_h: 固定为 1（不在特征维滑动）
          - k_w: 卷积核在时间维的宽度（这里使用 `self.kernel_size=2`）
        - 输出形状: (batch, output_dim, 1, seq_len - k_w + 1)
        """
    ...
    def SemanticClassify(self, data_pair, feedback, context, batch_norm=False):
        """融合当前响应序列编码与上下文、下一轮反馈隐状态，得到语义判别分数。

        - 参数:
          - data_pair: (batch_size, y_len, hidden_size) LSTM 的逐步输出
          - feedback: (batch_size, hidden_size) 下一轮反馈的末层隐状态
          - context: (batch_size, hidden_size) 对话上下文编码
          - batch_norm: 是否在卷积输出与全连接后使用 BN 稳定训练
        - 返回: (batch_size, 1) 未归一化的判别分数
        - 流程: Conv2d(2-gram) → ReLU → MaxPool(时间维全局池化) → 融合 context/feedback/bias → FC → Logits
        """
    ...
    def forward(self, neg, pos, fdb, context, emotion_context=None):
        ...
        - 输入:
          - neg: 负样本响应的 token 序列 (batch, y_len)
          - pos: 正样本响应的 token 序列 (batch, y_len)
          - fdb: 下一轮反馈的 token 序列 (batch, f_len)
          - context: 上下文编码 (batch, hidden)
        - 输出:
          - disc_sem_loss: 判别器的 WGAN 损失（含梯度惩罚）
          - gen_sem_loss: 供生成器优化的对抗项（鼓励判别器将负样本判为正）
        - 步骤:
          1) 嵌入+LSTM 得到序列输出
          2) 语义打分（负/正）并构造 WGAN 损失
          3) WGAN-GP 梯度惩罚，提升训练稳定性
        """
...
class Emotional_Discriminator(nn.Module):
    ...
    结构与语义判别器一致，但融合的是情感上下文 `emotion_context` 与反馈隐状态，
    用于衡量当前响应在情感表达上的合理性与一致性。
    """
    def __init__(self, vocab, embedding_size, hidden_size, num_layers, layer_dropout=0.4):
    ...
    def conv2d(self, input, output_dim, k_h, k_w):
        """同语义判别器：在时间维执行 2-gram 卷积，提取局部情感线索。"""
        '''
    ...
    def EmotionClassify(self, data_pair, feedback, context, emotion_context, batch_norm=False):
        """融合响应序列与情感上下文/反馈隐状态，得到情感判别分数。

        - 参数:
          - data_pair: (batch, y_len, hidden)
          - feedback: (batch, hidden)
          - context: (batch, hidden) 该参数保留以保持接口一致，不直接使用在情感融合处
          - emotion_context: (batch, hidden)
        - 返回: (batch, 1)
        - 流程: Conv2d → ReLU → MaxPool → 融合 emotion_context/feedback/bias → FC → Logits
        """
    ...
    def forward(self, neg, pos, fdb, context, emotion_context):
        ...
        - 输入: 同语义判别器，额外使用 `emotion_context`
        - 输出: (disc_emo_loss, gen_emo_loss)
        - 训练: 使用 WGAN-GP 与梯度惩罚稳定对抗训练
        """
        # 1 LSTM  - negative prediction
        embed_neg = self.embedding(neg)
        outputs_neg, hidden_neg = self.lstm(embed_neg)
...
class EmpDG_D(nn.Module):
    ...
    聚合语义与情感两个判别器，提供单步训练接口 `train_one_batch`。
    仅承担判别器部分的参数优化，生成器相关模块并不在此类中定义/维护。
    """
    ...
    def save_model(self, running_avg_ppl, iter, f1_g, f1_b, ent_g, ent_b):
        ...
        - 文件名包含: 迭代步、PPL/F1/Entropy 指标，便于离线对比
        - 注意: 本类实际并未定义 `encoder/decoder/generator` 成员，若需保存请确认外部注入
        """
        state = {
            'iter': iter,
    ...
    def train_one_batch(self, context, emotion_context=None, batch=None, train=False, iter_train=False):
        ...
        - 参数:
          - context: (batch, hidden) 上下文编码
          - emotion_context: (batch, hidden) 情感上下文编码（可选）
          - batch: dict，需包含以下键：
            - pred_batch, target_batch, feedback_batch
            - 若启用情感判别器，还需：pred_emotion_batch, target_emotion_batch, feedback_emotion_batch
          - train: True 表示标准一次反向与更新
          - iter_train: True 表示保留计算图的多次迭代更新（对抗训练中常用）
        - 返回: (loss_d.item(), loss_g.item())
        - 说明: `loss_d` 为判别器损失（含语义/情感与 GP），`loss_g` 为生成器对抗项累计
        """
