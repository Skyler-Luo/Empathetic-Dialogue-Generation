...
class Beam():
    """
    Beam搜索类，用于在解码时维护多个假设（beam）及其分数。

    属性:
        size (int): beam的大小。
        scores (Tensor): 当前各假设的分数。
        prev_ks (List[Tensor]): 每个时间步的前向指针。
        next_ys (List[Tensor]): 每个时间步生成的词索引。
    方法:
        advance(word_prob): 更新beam状态并检查是否完成。
        get_current_state(): 获取当前时间步的假设序列。
        get_hypothesis(k): 回溯生成第k个完整假设。
    """
...
class Translator(object):
    """
    Translator类，用于加载训练好的模型并执行Beam搜索解码。

    参数:
        model (nn.Module): 训练好的序列生成模型。
        lang (Language): 包含词汇表和索引映射的语言对象。
        beam_size (int): Beam大小，从config.beam_size获取。
        device (torch.device): 运行设备(cuda或cpu)。
    方法:
        beam_search(batch, max_dec_step): 对输入batch执行Beam搜索并返回生成句子列表。
    """
...
class LabelSmoothing(nn.Module):
    """
    标签平滑类，用于在计算KL散度损失时对目标分布进行平滑处理。

    参数:
        size (int): 类别总数。
        padding_idx (int): 填充索引，不参与平滑。
        smoothing (float): 平滑系数，表示将部分概率分散到其他类别。
    方法:
        forward(x, target): 计算带平滑的KL散度损失。
    """
...
class Embeddings(nn.Module):
    """
    词嵌入类，封装nn.Embedding并对输出进行缩放以匹配模型维度。
    ...
    方法:
        forward(x): 返回嵌入表示，大小为(batch_size, seq_len, d_model)。
    """
...
class NoamOpt:
    """
    Noam学习率调度器，实现《Attention Is All You Need》论文中的动态学习率策略。

    参数:
        model_size (int): 模型隐藏层维度。
        factor (float): 缩放因子。
        warmup (int): 预热步数。
        optimizer (Optimizer): 基础优化器实例。
    方法:
        step(): 更新参数并根据Noam公式调整学习率。
        rate(step=None): 计算并返回当前学习率。
    """
