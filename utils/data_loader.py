...
class Dataset(data.Dataset):
    """
    Dataset类继承自torch.utils.data.Dataset，用于对话数据加载和预处理。
    主要功能：
    1. 存储数据和词汇表
    2. 处理上下文、情感上下文、目标文本和反馈文本
    3. 将单词转换为索引，并处理OOV词汇
    4. 生成模型输入所需张量，包括mask等
    """
    def __init__(self, data, vocab):
        """
        构造函数，初始化数据集对象
        参数：
        - data (dict): 原始数据字段，包括"context", "emotion_context", "target", "emotion", "feedback", "situation"
        - vocab (object): 词汇表对象，包含word2index等映射
        """
        self.vocab = vocab
        self.data = data
        self.emo_map = {
    ...
    def target_oovs(self, target, oovs):  #
        ...
        针对目标文本生成扩展索引，处理OOV词
        参数：
        - target (list[str]): 目标单词列表
        - oovs (list[str]): 输入上下文中的OOV单词列表
        返回：
        - torch.LongTensor: 目标索引张量，含EOS
        """
        ids = []
        for w in target:
    ...
    def process_oov(self, context, emotion_context):  # oov for input
        """
        处理输入上下文和情感上下文中的OOV词并生成扩展索引
        参数：
        - context (list[list[str]]): 对话上下文
        - emotion_context (list[str]): 情感上下文单词列表
        返回：
        - ids (list[int]): 对话输入索引列表
        - ids_e (list[int]): 情感输入索引列表
        - oovs (list[str]): OOV单词列表
        """
        ids = []
        ids_e = []
        oovs = []
    ...
    def preprocess(self, arr, anw=False):
        ...
        将单词序列转换为索引序列
        参数：
        - arr: 输入序列；非anw模式下为[context, emotion_context]，anw模式下为单词列表
        - anw (bool): 是否为答案模式；True时仅转换并添加EOS
        返回：
        - 对应的torch.LongTensor或多项元素元组，用于模型输入
        """
        if anw:
    ...
    def preprocess_emo(self, emotion, emo_map):
        """
        将情感标签转换为one-hot向量及标签索引
        参数：
        - emotion (str): 情感标签名称
        - emo_map (dict): 情感到索引的映射
        返回：
        - program (list[int]): one-hot向量
        - index (int): 情感标签索引
        """
...
def collate_fn(batch_data):
    """
    自定义collate函数，用于DataLoader按batch合并数据
    参数：
    - batch_data (list[dict]): Dataset返回的数据列表
    返回：
    - dict: 合并后的输入输出张量及原始文本、长度等信息
    """
...
def prepare_data_seq(batch_size=32, adver_train=False, gen_data=False):
    """
    构建Train/Val/Test的DataLoader并写入配置
    参数：
    - batch_size (int): batch大小，gen_data=False时test使用1
    - adver_train (bool): 是否使用对抗训练数据
    - gen_data (bool): 是否生成全部测试集数据
    返回：
    - data_loader_tra, data_loader_val, data_loader_tst, vocab, emo_class_num (int)
    """
