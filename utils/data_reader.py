...
emotion_lexicon = json.load(open('datasets/empathetic-dialogue/NRCDict.json'))[0]


class Lang:
    """
    词汇表管理类，维护单词到索引、索引到单词映射，并统计单词频次。
    """
    def __init__(self, init_index2word):
        """
        构造函数，初始化词汇映射和计数字典。
        参数：
        - init_index2word (dict): 初始索引到单词的映射 {index: word}。
        """
    ...
    def index_words(self, sentence):
        """
        对句子中的每个单词调用index_word进行索引。
        参数：
        - sentence (list[str]): 单词列表。
        """
...
def get_wordnet_pos(tag):
    """
    将NLTK的词性标签映射为WordNet词性。
    参数：
    - tag (str): NLTK词性标签（如'NN','VB'等）。
    返回：
    - WordNet词性常量或None
    """
    if tag.startswith('J'):
...
def read_langs_for_D(vocab):
    """
    读取并预处理对话数据，生成训练/验证/测试集。
    参数：
    - vocab (Lang): 词汇表对象
    返回：
    - data_train, data_dev, data_test (dict): 各数据集字典
    - vocab (Lang): 更新后的词汇表
    """
...
def load_dataset(adver_train=False):
    """
    从预处理pickle文件加载empathetic-dialogue数据集。
    参数：
    - adver_train (bool): 是否显示对抗训练样例
    返回：
    - data_tra, data_val, data_tst (dict): 加载的数据字典
    - vocab (Lang): 词汇表对象
    """
    print("LOADING empathetic_dialogue ...")
    with open('datasets/empathetic-dialogue/empdg_dataset_preproc.p', "rb") as f:
...
if __name__ == '__main__':
    ...
    with open('datasets/empathetic-dialogue/empdg_dataset_preproc.p', "wb") as f:
